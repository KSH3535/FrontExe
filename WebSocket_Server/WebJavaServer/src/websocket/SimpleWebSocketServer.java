...
//		ex.getStackTrace();
	...
	public void onMessage(WebSocket conn, String message) {
		...
		if(cmd.equals("login")) {
			...
			conn.send(ackObj.toString());		//json 문자열로 변환되어서 클라이언트한테 전송됨
	...
	public void onOpen(WebSocket conn, ClientHandshake handshake) {
		...
		conn.send(ackObj.toString()); 		//접속하자마자 클라이언트한테 메시지 보내기
	}
